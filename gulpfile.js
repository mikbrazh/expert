...
gulp.task('buildcss', function() {
  ...
  .pipe(rename({ suffix: '.min', prefix : '' })) // Префикс
  .pipe(cleancss( {level: { 1: { specialComments: 0 } } })) // Минификация
...
gulp.task('buildvendorjs', function() {
  return gulp.src([
  ''+srcFolder+'/libs/jquery/dist/jquery.min.js', // JS библиотеки
    ])
  .pipe(concat('vendor.min.js')) // Конечный файл
...
gulp.task('buildscriptjs', function() {
  return gulp.src([
    ''+srcFolder+'/js/script.js' // Основной JS файл
    ])
  .pipe(concat('script.js')) // Конечный файл
  // .pipe(uglify()) // Минификация
  .pipe(gulp.dest(''+distFolder+'/js'))
  .pipe(browserSync.reload({ stream: true }))
});
